colors:
  ...
  # ── OPERA semantic aliases ──────────────────────────────────────────────────
  # Maps existing tokens to OPERA's four-stage pipeline.
  # Never introduce new hex values — reference tokens above only.

  opera-dump-surface: "{colors.surface-soft}"         # Mind Dump textarea bg — warmest, lowest stimulation
  opera-profiler-surface: "{colors.surface-card}"     # Profiler summary card bg
  opera-council-surface: "{colors.canvas}"            # Council Room main bg — breathing room
  opera-verdict-surface: "{colors.surface-soft}"      # Verdict card bg — calm resolution

  opera-conflict-flag: "{colors.warning}"             # Contradiction highlight — amber, not alarm-red
  opera-commit-cta: "{colors.primary}"                # The ONE coral moment — Commit button only
  opera-commit-confirmed: "{colors.success}"          # Post-commit state — calm green

  opera-persona-a: "{colors.accent-teal}"             # Rational/analytical archetype — cool, considered
  opera-persona-b: "{colors.accent-amber}"            # Pragmatic archetype — grounded, warm
  opera-persona-c: "{colors.primary}"                 # Emotional/values archetype — coral at 40% opacity in use

  # Usage rules:
  # - Dark surfaces (surface-dark, surface-dark-elevated) → footer only, never in main session flow
  # - opera-commit-cta (coral) → Commit button ONLY. No other coral in session screens.
  # - opera-persona-c rendered at opacity: 0.4 over canvas to keep it soft, not dominant
  # - conflict flags use opera-conflict-flag as left-border accent on warning card, not full background fill
...
components:
  ...
  opera-dump-textarea:
    backgroundColor: "{colors.opera-dump-surface}"
    textColor: "{colors.ink}"
    typography: "{typography.body-md}"
    rounded: "{rounded.lg}"
    padding: 24px
    ...
    focusRing: "0 0 0 3px rgba(204,120,92,0.12)"
    # Full width; min-height 240px; no label — input is the stage

  opera-profiler-card:
    backgroundColor: "{colors.opera-profiler-surface}"
    textColor: "{colors.ink}"
    typography: "{typography.body-md}"
    rounded: "{rounded.lg}"
    padding: 32px
    # Conflict flags rendered as left-border accent: 3px solid {colors.warning}

  opera-conflict-flag-card:
    backgroundColor: "{colors.surface-soft}"
    textColor: "{colors.ink}"
    typography: "{typography.body-sm}"
    rounded: "{rounded.md}"
    padding: "16px 20px"
    borderLeft: "3px solid {colors.opera-conflict-flag}"

  opera-persona-bubble:
    # Base — each persona overrides backgroundColor
    textColor: "{colors.ink}"
    typography: "{typography.body-md}"
    rounded: "{rounded.lg}"
    padding: 24px
    border: "1px solid {colors.hairline}"
    # Variants:
    # opera-persona-bubble-a: backgroundColor rgba(93,184,166,0.10) — teal wash
    # opera-persona-bubble-b: backgroundColor rgba(232,165,90,0.10) — amber wash
    # opera-persona-bubble-c: backgroundColor rgba(204,120,92,0.10) — coral wash

  opera-persona-label:
    typography: "{typography.caption-uppercase}"
    # Color matches persona accent: teal / amber / coral per archetype

  opera-verdict-card:
    backgroundColor: "{colors.opera-verdict-surface}"
    textColor: "{colors.ink}"
    typography: "{typography.body-md}"
    rounded: "{rounded.lg}"
    padding: 32px
  ...
  opera-pro-con-row:
    typography: "{typography.body-sm}"
    # Pro rows: left accent dot {colors.success}
    # Con rows: left accent dot {colors.error}

  opera-commit-button:
    backgroundColor: "{colors.opera-commit-cta}"
    textColor: "{colors.on-primary}"
    typography: "{typography.button}"
    rounded: "{rounded.md}"
    padding: "14px 32px"
    height: 48px
    # Larger than standard 40px — this is the ONE high-stakes action on screen

  opera-commit-confirmed-badge:
    backgroundColor: "{colors.opera-commit-confirmed}"
    textColor: "{colors.on-primary}"
    typography: "{typography.caption-uppercase}"
...
- **Dark surfaces in session flow are banned.** `surface-dark`, `surface-dark-elevated`, `surface-dark-soft` belong to footer and nav only. Every session screen (Dump → Profiler → Council → Verdict) stays on warm cream tones. Dark = cognitive load; OPERA's job is to reduce it.
- **Coral = Commit only.** `{colors.primary}` appears in one place per session screen: the Commit button on the Verdict screen. Persona C uses coral at 10% opacity as a background wash — not full coral fill.
- **Persona washes are opacity-based, not solid fills.** All three persona bubbles use the same `{rounded.lg}` card shape with a 10% tinted background. Visual distinction comes from the label color + left-border dot, not loud fills.
- **Conflict flags never use red.** `{colors.warning}` (amber) only. Error red (`{colors.error}`) is reserved for schema validation failures shown as system errors — never for user's logical contradictions.
- **The Commit button is 48px tall**, not the standard 40px. It is the only high-stakes irreversible action in the product.
- **No callout-card-coral in session screens.** The coral full-bleed callout is a marketing pattern. Inside the app, coral stays contained to the Commit button.
