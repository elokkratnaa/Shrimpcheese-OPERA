...
export default function AnalysisClient() {
  ...
  return (
    <motion.div
      ...
      className="bg-white/50 backdrop-blur-3xl border border-white/80 rounded-[2.5rem] p-8 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col gap-8 relative overflow-hidden"
    >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {analysis.key_themes.map((theme) => (
              <span
                key={theme}
                className="bg-white/80 border border-rose-100 text-rose-600 px-6 py-4 rounded-full text-[11px] tracking-widest"
              >
                {theme}
              </span>
            ))}
