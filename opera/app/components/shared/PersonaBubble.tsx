...
export default function PersonaBubble({
  ...
  return (
    <div className="flex flex-col gap-1 w-full animate-in fade-in slide-in-from-bottom-2 duration-200">
      ...
      <span className={`text-[12px] font-medium tracking-[1.5px] uppercase px-1 ${activeStyle.textAccent}`}>
        {persona_name}
      </span>

      {/* message bubble using shadcn Card */}
      <Card className={`rounded-lg border shadow-sm ring-0 ${activeStyle.bg} ${activeStyle.border}`}>
        <CardContent className="p-6">
          <p className="text-[#3d3d3a] text-[16px] leading-[1.55] whitespace-pre-wrap">
            {message_content}
            {isStreaming && (
              <span className="inline-block w-[3px] h-[15px] bg-[#cc785c] ml-1 animate-pulse" />
            )}
          </p>
        </CardContent>
      </Card>
