...
const dict = {
  en: {
    ...
    heroSub: "It doesn't need to be neat. It doesn't need to be perfect. Just pour out whatever is in your head.",
    ...
    categories: {
      "Karir": "Career", "Percintaan": "Romance", "Keluarga": "Family", "Keuangan": "Finance", "Pendidikan": "Education", "Self-Growth": "Self-Growth", "Lainnya": "Others"
    },
    emotions: {
      "anxious": "Anxious", "avoidant": "Avoidant", "risk-tolerant": "Risk Tolerant", "fatigued": "Fatigued", "hopeful": "Hopeful", "bingung": "Confused"
    },
    personas: {
  ...
  id: {
    ...
    heroSub: "Tidak perlu rapi. Tidak perlu sempurna. Tuliskan saja apa yang ada di kepalamu.",
    ...
    categories: {
      "Karir": "Karir", "Percintaan": "Percintaan", "Keluarga": "Keluarga", "Keuangan": "Keuangan", "Pendidikan": "Pendidikan", "Self-Growth": "Self-Growth", "Lainnya": "Lainnya"
    },
    emotions: {
      "anxious": "Cemas", "avoidant": "Menghindar", "risk-tolerant": "Berani", "fatigued": "Lelah", "hopeful": "Penuh Harap", "bingung": "Bingung"
    },
    personas: {
      "The Pragmatic Stoic": "Fokus pada kestabilan dan keputusan jangka panjang.",
      "The Venture Capitalist": "Mencari peluang terbesar di tengah ketidakpastian.",
      "The Creative Hedonist": "Mengutamakan makna, kebahagiaan, dan kualitas hidup.",
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-6 pb-20 pt-24 md:pt-32 flex flex-col relative z-10">
        ...
        >
          ...
          <div className="flex justify-between items-center mt-6 pt-6 border-t border-slate-200/50">
             <span className={`text-[10px] font-bold tracking-widest uppercase transition-colors ${mindDump.length >= 50 ? 'text-emerald-500' : 'text-slate-400'}`}>
        ...
        >
          <div className="text-center mb-2">
             <h2 className="text-3xl font-light font-serif text-slate-800">{t.contextSection}</h2>
             <p className="text-sm text-slate-500 font-light mt-3">{t.contextSub}</p>
          </div>
        ...
        >
          <div className="text-center">
             <h2 className="text-3xl font-light font-serif text-slate-800">{t.councilSection}</h2>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {PERSONA_POOL.map(key => {
              ...
              const customDesc = t.personas[persona.name as keyof typeof t.personas] || persona.description;
              ...
              return (
                ...
                >
                  {isSelected && <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/10 rounded-full blur-3xl -mr-10 -mt-10" />}

                  <div className="flex items-center justify-between mb-6 relative z-10">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl font-serif italic border-2 border-white shadow-sm ${
                      isSelected ? "bg-gradient-to-tr from-indigo-100 to-rose-100 text-indigo-700" : "bg-slate-100 text-slate-400 group-hover:bg-slate-200 group-hover:text-slate-600 transition-colors"
                    }`}>
                      {friendlyName.charAt(0)}
                    </div>
                    {squadMode === "racik" && (
                      <div className={`w-6 h-6 rounded-full border-[1.5px] flex items-center justify-center transition-all ${
                        isSelected ? "bg-indigo-500 border-indigo-500" : "border-slate-300 bg-white/50"
                      }`}>
                  ...
                  <div className="relative z-10 flex-1 flex flex-col justify-end">
                    <h4 className="text-lg font-serif text-slate-900 mb-2">{friendlyName}</h4>
        ...
        {/* ================================================================= */}
        {errorMessage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-8 text-xs font-bold tracking-widest text-rose-500 bg-rose-50 border border-rose-100 px-6 py-4 rounded-xl text-center uppercase">
            {errorMessage}
          </motion.div>
        )}
