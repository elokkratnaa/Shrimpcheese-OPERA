...
const dict = {
  en: {
    ...
    headerSub: "Track your reflections, discover recurring patterns, and understand how your decision-making evolves over time.",
    identity: "Identity",
    ...
    deleteConfirm: "Account deletion must be processed by an administrator or through your old dashboard settings. Please confirm to log out.",
  ...
  id: {
    ...
    headerSub: "Lacak refleksimu, temukan pola berulang, dan pahami bagaimana pengambilan keputusanmu berkembang seiring waktu.",
    identity: "Identitas",
    ...
    deleteConfirm: "Penghapusan akun saat ini memerlukan proses admin. Silakan logout atau hubungi dukungan.",
...
export default function ProfileClient({ initialName, email, provider, stats }: ProfileProps) {
  ...
  return (
    <div className="flex flex-col gap-12 md:gap-16">
      ...
      <div className="grid lg:grid-cols-3 gap-8 items-start">
            ...
            {isEditing ? (
              ...
              >
                <h2 className="text-2xl md:text-3xl font-serif text-slate-900 group-hover:text-[#6366F1] transition-colors flex items-center gap-2">
                  {name || "User"}
                  <svg className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                </h2>
        ...
        <div className="lg:col-span-2 grid grid-cols-2 gap-6">
          ...
          >
            ...
            <div>
              {hasData ? (
                <p className="text-5xl md:text-6xl font-light font-serif text-slate-900 mb-3 tracking-tight">{stats.totalSessions}</p>
              ) : (
                <p className="text-xl md:text-2xl font-serif text-slate-900 mb-3 leading-tight">{t.startJourney}</p>
              )}
              <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500">{t.totalSessions}</p>
            </div>
          </motion.div>
          ...
          >
            ...
            <div>
              {hasData ? (
                <p className="text-5xl md:text-6xl font-light font-serif text-slate-900 mb-3 tracking-tight">{stats.committedCount}</p>
              ) : (
                <p className="text-xl md:text-2xl font-serif text-slate-900 mb-3 leading-tight">{t.noReflections}</p>
              )}
              <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500">{t.resolutions}</p>
            </div>
          </motion.div>
        </div>
      </div>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              ...
              <div>
                {hasData ? (
                  <>
                    <p className="text-3xl md:text-4xl font-light font-serif text-slate-900 mb-3 tracking-tight">Sage</p>
                    <p className="text-sm text-slate-500 font-light leading-relaxed mb-4">{t.sageDesc}</p>
                  </>
                ) : (
                  <p className="text-xl md:text-2xl font-serif text-slate-900 mb-4 leading-tight">{t.notEnough}</p>
                )}
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400">{t.alignedPersona}</p>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div className="w-12 h-12 rounded-full bg-white/80 flex items-center justify-center text-indigo-600 mb-8 shadow-sm group-hover:scale-110 transition-transform">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" /></svg>
              </div>
              <div>
                {stats.topTag ? (
                ...
                <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400">{t.topTheme}</p>
              </div>
            </div>
          </motion.div>
      ...
      >
        <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">{t.management}</h3>
